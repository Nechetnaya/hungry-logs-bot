...
async def parse_meal_text(text: str, user_id: int) -> dict:
    ...
    Если ты не можешь даже примерно определить продукты или вес порции, 
    верни JSON только с полем "clarification" с вопросом к пользователю, чтобы уточнить недостающие данные.

    Не задавай уточняющий вопрос, если информации достаточно для расчёта.
